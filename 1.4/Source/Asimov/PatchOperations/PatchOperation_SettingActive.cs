...
namespace Asimov
{
    public class PatchOperation_SettingActive : PatchOperation
    {
        ...
        public override bool ApplyWorker(XmlDocument xml)
        {
            ...
            ModContentPack modContentPack = AsimovMod.mod.Content;
            for (int i = 0; i < settings.Count(); i++)
            {
                if (!AsimovMod.settings.IsValidSetting(settings[i]))
                {
                ...
                if (AsimovMod.settings.GetEnabledSettings.Contains(settings[i]))
                {
                    flag = true;
                    break;
                }
