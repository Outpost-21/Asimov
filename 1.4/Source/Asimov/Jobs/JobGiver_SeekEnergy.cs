    ...
    public class JobGiver_SeekEnergy : ThinkNode_JobGiver
    {
        ...
        public override Job TryGiveJob(Pawn pawn)
        {
            ...
            if (modExt.canChargeWirelessly)
            {
                ...
                {
                    localWireless.PullEnergyFromChargers(pawn, (energy.MaxLevel - energy.CurLevel * 1000));
                    if (energy.CurCategory < EnergyCategory.GettingLow)
                    {
                        return null;
                    }
                ...
                {
                    worldWireless.PullEnergyFromChargers(pawn, energy.MaxLevel - energy.CurLevel * 1000);
                    if (energy.CurCategory < EnergyCategory.GettingLow)
                    {
                        return null;
                    }
            ...
            if (modExt.canChargeFromChargepacks)
            {
                ...
                if (pawn.carryTracker is Pawn_CarryTracker carryTracker && carryTracker.CarriedThing is Thing carriedThing && carriedThing.TryGetComp<Comp_EnergySource>() != null)
                {
                    ...
                    {
                        return new Job(AsimovDefOf.Asimov_ConsumeEnergySource, new LocalTargetInfo(carriedThing))
                        {
                ...
                if (pawn.inventory is Pawn_InventoryTracker inventory && inventory.innerContainer.Any(thing => thing.TryGetComp<Comp_EnergySource>() != null))
                {
                    ...
                    if (validEnergySource != null)
                    {
                        ...
                        if (thingCount > 0)
                        {
                            return new Job(AsimovDefOf.Asimov_ConsumeEnergySource, new LocalTargetInfo(validEnergySource))
                            {
                                count = thingCount
                            };
                        }
                ...
                if (closestConsumablePowerSource != null)
                {
                    ...
                    if (energySourceComp != null)
                    {
                        ...
                        if (thingCount > 0)
                        {
                            return new Job(AsimovDefOf.Asimov_ConsumeEnergySource, new LocalTargetInfo(closestConsumablePowerSource))
                            {
                                count = thingCount
                            };
                        }
            ...
            if (hibernationComp != null && hibernationComp.autoHibernate)
            {
                Thing hibernationSpot = EnergyUtil.GetClosestUnreservedHibernationSpot(pawn);
                if (hibernationSpot != null)
                {
                    Messages.Message("Asimov.LowEnergyHibernation".Translate(pawn.Name.ToString()), MessageTypeDefOf.NegativeEvent);
