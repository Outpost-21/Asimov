    ...
    public class JobDriver_RestorePower : JobDriver
	{
		...
		public override IEnumerable<Toil> MakeNewToils()
		{
			...
			this.FailOn(delegate
			{
				...
				return (pawn == Deliveree && pawn.Faction == Faction.OfPlayer && pawn.playerSettings != null) ? true : false;
			});
			AddEndCondition(delegate
			{
				...
				return ((job.playerForced || pawn.Faction != Faction.OfPlayer) && Deliveree.DiedFromPowerLoss()) ? JobCondition.Ongoing : JobCondition.Succeeded;
			});
			...
			yield return Toils_Repair.FinalizeRestorePower(Deliveree);
