...
namespace Asimov
{
    public static class SettingsUtil
    {
        ...
        public static void SettingsDropdown(this Listing_Standard listing, string name, string explanation, ref AsimovSettingsPage value, float width)
        {
            float curHeight = listing.CurHeight;
            Rect rect = listing.GetRect(Text.LineHeight + listing.verticalSpacing);
            Text.Font = GameFont.Small;
            ...
            if (Widgets.ButtonText(rect2, value.ToString().Replace("_", " "), true, true, true))
            {
                ...
                List<AsimovSettingsPage> enumValues = Enum.GetValues(typeof(AsimovSettingsPage)).Cast<AsimovSettingsPage>().ToList();
                foreach (AsimovSettingsPage enumValue in enumValues)
                {
                    list.Add(new FloatMenuOption(enumValue.ToString().Replace("_", " "), delegate ()
                    {
                        AsimovMod.mod.currentPage = enumValue;
                    }));
                }
